...
class SeaBattle {
  ...
  void startIgra() {
    ...
    if (korableyOstalsyaKompyuter <= 0) {
      print('\nВы победили!');
      pobeditel = 'Вы';
      winnerScore = potoplenoKorableyKompyuter; // Score - число потопленных кораблей противника
      remainingShips = korableyOstalsyaIgrok;  // Оставшиеся корабли у победителя
    } else {
      print('\nКомпьютер победил!');
      pobeditel = 'Компьютер';
      winnerScore = potoplenoKorableyIgrok; // Score - число потопленных кораблей противника
      remainingShips = korableyOstalsyaKompyuter; // Оставшиеся корабли у победителя
    }

    print('\n--- Финальное поле компьютера ---');
    printPolye(poleKompyutera);
    print('\n--- Финальное поле игрока ---');    printPolye(poleIgroka);
    ...
    statistic(winnerScore, remainingShips, pobeditel);
  }
  ...
  void printPolye(List<List<String>> pole) {
    ...
    for (int i = 0; i < razmerPolya; i++) {
      stdout 
.write('${String.fromCharCode('A'.codeUnitAt(0) + i)} ');
    }
    print('');
  ...
  void statistic(int winnerScore, int remainingShips, String winnerName) {
    final directory = Directory('staistic');
    if (!directory.existsSync()) {
      directory.createSync();
    }

    final file = File('${directory.path}/statistic.txt');
    file.writeAsStringSync('Победитель: $winnerName\n Финальный счет: $winnerScore\n Оставшиеся корабли: $remainingShips');

    print('Статистика игры сохранена в файле: ${file.path}');
  }
}